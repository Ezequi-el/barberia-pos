...
const Dashboard: React.FC<DashboardProps> = ({ onNavigate, onLogout }) => {
  ...
  const cards = [
    ...
    },
    {
      ...
      title: 'Clientes',
      icon: User,
      description: 'Gestión de clientes y visitas',
      color: 'text-zinc-100',
      borderColor: 'group-hover:border-zinc-500/50'
    },
    {
    ...
    <div className="h-screen flex flex-col">
      ...
      <div className="bg-zinc-900 border-b border-zinc-800 px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <span className="font-heading font-bold text-xl tracking-wider">NERON POS</span>
        </div>

        <div className="flex items-center gap-4 md:gap-6 w-full md:w-auto justify-between md:justify-end">
          {/* System Status */}
          <div className={`flex items-center gap-2 px-3 py-1.5 bg-zinc-950 rounded-full border border-zinc-800 shadow-sm ${!isOnline ? 'border-red-900/50 bg-red-900/10' : ''}`}>
            ...
            <span className={`text-xs font-bold uppercase tracking-wide ${isOnline ? 'text-zinc-300' : 'text-red-400'}`}>
